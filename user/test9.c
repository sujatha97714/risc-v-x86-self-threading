...
void ul_start_func(int a1) {
    ...
    for (int i = 0; i < 10000000; i++) {
        if (i%1000000 == 0) {
            if(i == 5000000 && get_current_tid() == 3) {
                uint64 args[6] = {0,0,0,0,0,0}; 
                ulthread_create((uint64) ul_start_func, (uint64) (stacks+((3+1)*PGSIZE)), args, 10);
            }
...
main(int argc, char *argv[])
{
    ...
    printf("[*] User-Level Threading Custom Test #9 (ROUNDROBIN-2) Complete.\n");
    return 0;
}
